...
describe("RateLimiter", () => {
  ...
  test("should handle errors in scheduled tasks", async () => {
    ...
    expect(
      rateLimiter.schedule(() => Promise.reject(error))
    ).rejects.toThrow("Test error");
  });

  test("should respect custom interval", async () => {
    process.env.NODE_ENV = 'development';
  ...
  test("should handle queue overflow", async () => {
    ...
    const promises = Array(maxConcurrent + 1).fill(0).map((_, i) =>
      limiter.schedule(() => Promise.resolve(i))
    );
