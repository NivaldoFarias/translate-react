...
describe("GitHubService", () => {
	describe("Mock Tests", () => {
		...
		test("should handle non-existent files", async () => {
			mockOctokit.rest.repos.getContent = mock(() => Promise.reject(new Error("Not found")));

			expect(github.getFileContent("nonexistent.md")).rejects.toThrow("Not found");
		});
		...
		test("should handle invalid branch names", async () => {
			...
			const branchName = await github.createTranslationBranch(mockFile.path);
		...
		test("should handle API rate limit errors", async () => {
			...
			expect(github.getUntranslatedFiles()).rejects.toThrow("rate limit");
		});

		test("should handle network errors", async () => {
			mockOctokit.rest.repos.getContent = mock(() => Promise.reject(new Error("Network error")));

			expect(github.getGlossary()).rejects.toThrow("Network error");
		});
	});
	...
	describe("Live API Tests", () => {
		...
		test(
			"should fetch up to 10 untranslated files",
			async () => {
		...
		test("should commit translation", async () => {
			...
			expect(github.commitTranslation(branch, testFile, translation)).resolves.toBeUndefined();
