...
const RU_SEGMENT_BATCH_CONTEXT = `
...
const RU_STYLE_AND_SEMANTIC_RULES = `
...
- English 'As dependencies of \`RichTextEditor\`, \`formatDate\` and \`Button\`' must name only \`formatDate\` and \`Button\` as dependencies of \`RichTextEditor\`, e.g. '\`formatDate\` и \`Button\`, будучи зависимостями \`RichTextEditor\`, также будут ...'`;

const RU_DEPRECATED_RULES = `
...
const RU_SPECIFIC_RULES = `
# RUSSIAN SPECIFIC RULES
${RU_DEPRECATED_RULES}
${RU_MDN_URL_RULE}
${RU_STYLE_AND_SEMANTIC_RULES}`;

const RU_SEGMENT_SPECIFIC_RULES = `
# RUSSIAN SPECIFIC RULES
${RU_SEGMENT_BATCH_CONTEXT}
${RU_DEPRECATED_RULES}
${RU_STYLE_AND_SEMANTIC_RULES}`;
...
export const ruLocale: LocaleDefinition = {
	...
	rules: {
		specific: RU_SPECIFIC_RULES,
		segmentSpecific: RU_SEGMENT_SPECIFIC_RULES,
	},
	pullRequest: {
