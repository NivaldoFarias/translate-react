...
- Changes under `src/app/services/translator/`, `src/app/services/runner/`, or `src/app/locales/` trigger a required `ci.yml` smoke gate: `ci:smoke -- --profile quick --lang pt-br` (real LLM, mocked GitHub) before merge.
- Other configured locales run as optional matrix jobs that do not block merge. Run locally first with `bun run ci:smoke -- --profile quick --lang <locale>` before pushing. See [Workflow smoke](#workflow-smoke) for outputs and CI artifacts.
...
- Adding a production locale: follow the [wiki checklist](https://github.com/NivaldoFarias/translate-react/wiki/Workflow#adding-a-locale) (`registry.ts`, locale module layout, parity test, fork secrets, dry run).
...
Local runs and manual [`smoke.yml`](./.github/workflows/smoke.yml) dispatch write to gitignored `artifacts/smoke/`. Each fixture gets a subdirectory (for example `use-memo/`) with `translated.md` and `pull-request.md`. When the run posts progress, `translation-progress-issue-comment.md` sits at the `artifacts/smoke/` root. Override with `--out-dir`/`-o` or `SMOKE_OUTPUT_DIR`.
...
Use `gh run download <run_id> --dir artifacts/smoke/<run_id>` for smoke outputs. For translation logs, download the `translation-logs-*` artifact the same way (tree includes `logs/`).
